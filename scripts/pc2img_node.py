...
class ProjectionNode:
  def __init__(self):
    ...
    self.camera_matrix =  np.array([[7.215377000000e+02, 0.000000000000e+00, 6.095593000000e+02],
    ...
    # the camera has no distortion (np.array([k1, k2, p1, p2, k3]))
    self.dist_coeffs = np.array([0, 0, 0, 0, 0])

    # Set the rotation matrix and translation vector
    # This will need to be evaluated once we know the position
    # of the camera and the lidar on the car
    self.rmatr =  np.array([[7.533745000000e-03, -9.999714000000e-01, -6.166020000000e-04],
                            [-4.069766000000e-03, 1.480249000000e-02, 7.280733000000e-04],
                            [-9.998902000000e-01, -7.631618000000e-02, 9.998621000000e-01]])
    self.tvec = np.array([7.523790000000e-03, 1.480755000000e-02, -2.717806000000e-01])

    # The lookup table for the colors
    self.color_lut = None
  ...
  def project_point_cloud(self, point_cloud, image):
    ...
    camera_points, _ = cv2.projectPoints(homogeneous_points, self.rmatr, self.tvec, self.camera_matrix, self.dist_coeffs)
  ...
  def create_color_lut(self):
    ...
    distances = np.linalg.norm(self.point_cloud.reshape(3,3), axis=1)
